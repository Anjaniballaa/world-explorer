...
router.get("/", async (req, res) => {
  ...
  try {
    ...
    // Step 3: Get Wikipedia page for the CITY specifically (not state)
    let cityWiki = null;
    ...
    if (citySearchTerm) {
      try {
        // Search for the city page directly
        ...
        // Extract language from city Wikipedia
    ...
    if (primaryLang && citySearchTerm) {
      ...
      if (langCode) {
        try {
          // Search for city name in native language Wikipedia
          const nativeRes = await axios.get(
            `https://${langCode}.wikipedia.org/api/rest_v1/page/summary/${encodeURIComponent(citySearchTerm)}`,
            { headers: { "User-Agent": "WorldExplorerApp/1.0" }, timeout: 5000 }
          );
          // Only use if the page is actually about the city (not a redirect to state)
          const nativeTitle = nativeRes.data.title || "";
          nativeScript = {
            nativeCityName: nativeTitle,
            langCode,
